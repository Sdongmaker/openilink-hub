...
func (db *DB) EnsureRelayMember(botID string) (string, error) {
	...
	var emoji string
	err := db.QueryRow("SELECT emoji FROM relay_members WHERE bot_id = ?", botID).Scan(&emoji)
	if err == nil {
		return emoji, nil
	}
	...
	_, err = db.Exec("INSERT INTO relay_members (bot_id, emoji, joined_at) VALUES (?, ?, ?)", botID, emoji, now)
	if err != nil {
		// Race: another goroutine inserted first. Read theirs.
		var existing string
		if err2 := db.QueryRow("SELECT emoji FROM relay_members WHERE bot_id = ?", botID).Scan(&existing); err2 == nil {
			return existing, nil
		}
...
func (db *DB) GetRelayEmoji(botID string) string {
	var emoji string
	db.QueryRow("SELECT emoji FROM relay_members WHERE bot_id = ?", botID).Scan(&emoji)
	return emoji
}

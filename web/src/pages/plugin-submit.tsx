    ...
    <div className="space-y-4">
      ...
      <Card className="flex items-start gap-3 bg-primary/5 border-primary/20">
        ...
        <div className="text-xs">
          ...
          <p className="text-muted-foreground mt-0.5">
            ...
            链接发给 AI 助手，描述你的需求，AI 会生成完整的插件代码。生成后粘贴到下方提交即可。
          </p>
        </div>
      </Card>
      ...
      <Card className="space-y-3">
        ...
        <div className="flex border rounded-lg overflow-hidden w-fit">
          <button
            className={`px-3 py-1 text-xs cursor-pointer ${mode === "github" ? "bg-secondary" : "text-muted-foreground"}`}
            onClick={() => setMode("github")}
          >
            GitHub 链接
          </button>
          <button
            className={`px-3 py-1 text-xs cursor-pointer ${mode === "paste" ? "bg-secondary" : "text-muted-foreground"}`}
          ...
          {mode === "github" ? (
            <>
              ...
              <p className="text-xs text-muted-foreground">
                自动拉取脚本并固定 commit hash，确保审核的代码就是运行的代码。
              </p>
            </>
          ) : (
            <>
              ...
              />
              <p className="text-xs text-muted-foreground">
                使用 <code className="bg-secondary px-1 rounded">{"// ==WebhookPlugin=="}</code>{" "}
                格式声明元数据。
                <a
                  href="/api/webhook-plugins/skill.md"
                  target="_blank"
      ...
      <Card className="space-y-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
          <div className="p-2 rounded border bg-background">
            ...
            <p className="text-muted-foreground">
              .sender .content .msg_type .channel_id .bot_id .timestamp .items[]
            </p>
          </div>
          <div className="p-2 rounded border bg-background">
        ...
        <div className="flex items-center gap-3 text-xs text-muted-foreground">
          <span>
            <Shield className="w-3 h-3 inline" /> 5s 超时
          </span>
          <span>栈深 64</span>
          ...
          <a
            href="/api/webhook-plugins/skill.md"
            target="_blank"
            className="text-primary hover:underline ml-auto flex items-center gap-0.5"
          >
